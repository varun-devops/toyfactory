...
export default function Navbar() {
  ...
  return (
    <nav style={{
      ...
      boxShadow: shadow ? '0 2px 20px rgba(0,0,0,0.09)' : '0 1px 0 #f0f0f0',
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 20px', display: 'flex', alignItems: 'center', height: 64, gap: 12 }}>
        ...
        <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 9, color: '#aaa', fontWeight: 700, letterSpacing: '0.08em' }}>INDIA&apos;S #1 TOY STORE</div>
          </div>
        </Link>

        {/* Desktop search */}
        <form onSubmit={search} style={{ flex: 1, maxWidth: 480 }} className="hidden md:flex">
          <div style={{ position: 'relative', width: '100%' }}>
            <input
              ...
              onFocus={e => (e.target.style.boxShadow = '0 0 0 3px rgba(255,107,0,0.15)')}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginLeft: 'auto' }}>
          ...
          <Link href="/favorites" style={{
            ...
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            borderRadius: 12,
            background: favorites.length ? '#fff0f5' : 'transparent',
          ...
          <Link href="/cart" style={{
            ...
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            borderRadius: 12,
            background: cartCount ? '#fff3ea' : 'transparent',
            transition: 'background 0.15s',
          }}>
          ...
          <button
            ...
            className="md:hidden"
            style={{ width: 42, height: 42, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 12, background: open ? '#f5f5f5' : 'transparent' }}
      ...
      <div className="hidden md:block" style={{ borderTop: '1px solid #f0f0f0', overflowX: 'auto' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 20px', display: 'flex', gap: 2, height: 42, alignItems: 'center' }}>
          {CATS.map(([label, href]) => (
      ...
      {/* ── Mobile search bar ── */}
      <form onSubmit={search} className="md:hidden" style={{ padding: '0 16px 14px' }}>
      ...
      {/* ── Mobile menu ── */}
      {open && (
        <div style={{ background: '#fff', borderTop: '1px solid #f0f0f0', padding: '12px 16px 24px' }}>
          ...
          <div style={{ borderTop: '1px solid #f0f0f0', marginTop: 12, paddingTop: 12, display: 'flex', flexDirection: 'column', gap: 4 }}>
            {[['🛒 Cart', '/cart'], ['❤️ Favourites', '/favorites'], ['📦 Orders', '/orders']].map(([l, h]) => (
              <Link key={h} href={h} onClick={() => setOpen(false)} style={{
                display: 'block', padding: '10px 16px', borderRadius: 14,
